...
class TestUtils {
  ...
  public static String readTestFile(String file) {
    return new String(Files.readAllBytes(Paths.get(TestUtils.class.getResource(file).getFile())), StandardCharsets.UTF_8);
  }
  ...
  public static void createEmlSchemasExpectation(ClientAndServer clientAndServer) {
    clientAndServer.when(HttpRequest.request().withMethod("GET").withPath("/eml.xsd"))
      .respond(HttpResponse.response(readTestFile("/schemas/eml.xsd")));

    clientAndServer.when(HttpRequest.request().withMethod("GET").withPath("/eml-gbif-profile.xsd"))
      .respond(HttpResponse.response(readTestFile("/schemas/eml-gbif-profile.xsd")));

    clientAndServer.when(HttpRequest.request().withMethod("GET").withPath("/dc.xsd"))
